...
for filename in files:
    ...
    if order==1:
        congr_corr=   df['key_resp.corr'][24:44].dropna().tolist()  #corrs[0:block_length]
        congr_rts=    df['key_resp.rt'][24:44].dropna().tolist()   #rts[0:block_length]
        incon_corr=   df['key_resp.corr'][56:].dropna().tolist() #corrs[block_length:]
        incon_rts=    df['key_resp.rt'][56:].dropna().tolist()    #rts[block_length:]
    else:
        congr_corr= df['key_resp.corr'][56:].dropna().tolist()  #corrs[block_length:]
        congr_rts= df['key_resp.rt'][56:].dropna().tolist() #rts[block_length:]
        incon_corr= df['key_resp.corr'][24:44].dropna().tolist() #corrs[0:block_length]
        incon_rts= df['key_resp.rt'][24:44].dropna().tolist() #rts[0:block_length]
    ...
    if sum(np.array(congr_rts + incon_rts)<fastRT_limit)>len(congr_rts + incon_rts)*fast_prop_limit:
        ...
        summary.append([os.path.basename(filename),"{:+.3f}".format(IAT), "{:+.3f}".format(simpleIAT), "{:+.3f}".format(np.mean(congr_rts)),"{:+.3f}".format(np.std(congr_rts)),"{:+.2f}".format(1-(sum(congr_corr)/len(congr_corr))),"{:+.3f}".format(np.mean(incon_rts)),"{:+.3f}".format(np.std(incon_rts)),"{:+.2f}".format(1-(sum(incon_corr)/len(incon_corr))),"{:+.3f}".format(congr_adjmean),"{:+.3f}".format(incon_adjmean)])            
        # np.std(congr_rts)
