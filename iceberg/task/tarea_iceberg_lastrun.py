...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "random_routines" ---
    ...
    # --- Initialize components for Routine "nueva_ronda" ---
    ...
    trials_5 = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('imgs/routines_order.xlsx'),
    ...
    for thisTrial_5 in trials_5:
        ...
        # --- Prepare to start Routine "random_routines" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('random_routines.started', globalClock.getTime())
        # Run 'Begin Routine' code from code_4
        # Retrieve the name of the next routine from the loop
        ...
        # keep track of which components have finished
        random_routinesComponents = []
        for thisComponent in random_routinesComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "random_routines" ---
        for thisComponent in random_routinesComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('random_routines.stopped', globalClock.getTime())
        # Run 'End Routine' code from code_4
        # Check which routine should run next and set a flag for it
        ...
        # the Routine "random_routines" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "nueva_ronda" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('nueva_ronda.started', globalClock.getTime())
        text_7.setText(TextoRonda)
        # keep track of which components have finished
        nueva_rondaComponents = [text_7]
        for thisComponent in nueva_rondaComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "nueva_ronda" ---
        for thisComponent in nueva_rondaComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('nueva_ronda.stopped', globalClock.getTime())
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        for thisTrial in trials:
            ...
            # Run 'Begin Routine' code from control_starter
            # Verificar si la rutina debe ejecutarse basándose en la bandera correspondiente
            if control_started:
                continueRoutine = True
            else:
                continueRoutine = False
        ...
        for thisTrial_2 in trials_2:
            ...
            # Run 'Begin Routine' code from music_starter
            # Verificar si la rutina debe ejecutarse basándose en la bandera correspondiente
            if music_started:
                continueRoutine = True
            else:
        ...
        for thisTrial_3 in trials_3:
            ...
            # Run 'Begin Routine' code from music_words_starter
            # Verificar si la rutina debe ejecutarse basándose en la bandera correspondiente
            if music_words_started == True:
                continueRoutine = True
            if music_words_started == False:
                continueRoutine = False
            # Run 'Begin Routine' code from code_2
            selectedRows = random.sample(range(totalRows), nRows)
            selectedRowsStr = [i for i in selectedRows]
            background_movie.setAutoDraw(True)
        ...
        for thisTrial_4 in trials_4:
            ...
            # Run 'Begin Routine' code from music_video_starter
            # Verificar si la rutina debe ejecutarse basándose en la bandera correspondiente
            if music_video_started:
                continueRoutine = True
            else:
                continueRoutine = False
